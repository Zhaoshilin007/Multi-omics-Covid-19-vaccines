...
# annotated the lipid name into the super-class
...
for (vaccine_types in c("BNT","SNV")){
  if(vaccine_types=="BNT") {
   ...
   coldata_current <- data.frame(Time = factor(substr(names(df_rna_current), 7, 8)),
                                  ...
                                  batch = factor(substr(names(df_rna_current), 10, 11))) %>%
...
write.table(res.current.df, 
                paste0('~/Projects/Multi_omics/Results/2_Differential_analysis/Raw_data/M1_M0_Limma_Adjust_M6_M0_', vaccine_types,'.txt'), 
